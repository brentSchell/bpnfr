function E_c_NF_sup = E_c_PNF_sup( r_c_near,f,freq,I)
...
for rIndex=1:size(r_c_near,1);
    ...
    for fIndex = 1:size(f,1);
        ...
        thetap = acos(zp/rp);
        phip = atan(yp/xp);
        Ep_rp_NF = ((eta*I(fIndex)*len*cos(thetap))/(2*pi*rp^2))*(1+1/(1i*k*rp))*exp(-1i*k*rp);
        Ep_thetap_NF = (1i*eta*k*I(fIndex)*len*sin(thetap)/(4*pi*rp))*(1+1/(1i*k*rp)-1/(k^2*rp^2))*exp(-1i*k*rp);
        Ep_phip_NF = 0;
