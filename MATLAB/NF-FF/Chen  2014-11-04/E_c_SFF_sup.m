function E_c_FF_sup = E_c_SFF_sup( radius_FF,dTheta,dPsi,f,freq )
...
for r_cIndex=1:size(r_s_far,1)
    x = r_s_far(r_cIndex,1)*cos(r_s_far(r_cIndex,2))*sin(r_s_far(r_cIndex,3));
    y = r_s_far(r_cIndex,1)*sin(r_s_far(r_cIndex,2))*sin(r_s_far(r_cIndex,3));
    z = r_s_far(r_cIndex,1)*cos(r_s_far(r_cIndex,3));
